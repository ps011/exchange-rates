...
export async function fetchExchangeRates(): Promise<ExchangeRatesProps> {
  ...
  try {
    const res: Response = await fetch(
      `${process.env.CURRENCY_API_URL}/rates?key=${process.env.CURRENCY_API_KEY}`,
    );
...
export const getConvertedValue = (
  ...
  if (sourceCurrency && destinationCurrency) {
    return parseFloat(
      (
        value *
        (exchangeRates[destinationCurrency.code] /
...
export const getDefaultCurrency = (
  ...
  const code =
    (query as CurrencyCodes) || (localStorage.getItem(key) as CurrencyCodes);
  if (key === SRC_KEY) {
